...
function check_user_silent()
{
  ...
  if (!isset($_SESSION['userid']) && isset($_COOKIE['identifier']) && isset($_COOKIE['securitytoken'])) {
    ...
    setcookie("identifier", $identifier, time() + (10 * 365 * 24 * 60 * 60), "/", "", true, true); // 10 years validity, Secure, HttpOnly
    setcookie("securitytoken", $neuer_securitytoken, time() + (10 * 365 * 24 * 60 * 60), "/", "", true, true); // 10 years validity, Secure, HttpOnly
  ...
  // Extend the validity of identifier and securitytoken cookies on each successful login
  if (isset($_COOKIE['identifier']) && isset($_COOKIE['securitytoken'])) {
    setcookie("identifier", $_COOKIE['identifier'], time() + (10 * 365 * 24 * 60 * 60), "/", "", true, true);
    setcookie("securitytoken", $_COOKIE['securitytoken'], time() + (10 * 365 * 24 * 60 * 60), "/", "", true, true);
  }
...
function getProfilePicPath($userId, $pdo)
{
  ...
  if (!empty($result['profile_pic'])) {
    return '/intern/uploads/profile_pics/' . $result['profile_pic'];
  } else {
...
{
  return $user['profile_pic'] == null ? "user.png" : $user['profile_pic'];
}
