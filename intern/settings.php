...
if(isset($_GET['save'])) {
	...
	if($save == 'personal_data') {
		...
		if($vorname == "" || $nachname == "") {
			...
			$statement = $pdo->prepare("UPDATE users SET vorname = :vorname, nachname = :nachname, festnetz = :festnetz, mobil = :mobil, geburtsdatum = :geburtsdatum, updated_at=NOW() WHERE id = :userid");
			$result = $statement->execute(array('vorname' => $vorname, 'nachname'=> $nachname, 'festnetz' => $festnetz, 'mobil' => $mobil, 'geburtsdatum' => $geburtsdatum, 'userid' => $user['id'] ));
    ...
    <div role="tabpanel" class="tab-pane active" id="data">
    	...
    	<form action="?save=personal_data" method="post" class="form-horizontal" accept-charset="utf-8">
    		<div class="form-group">
    			...
    			<div class="col-sm-10">
    				<input class="form-control" id="inputVorname" name="vorname" type="text" value="<?php echo htmlentities($user['vorname']); ?>" x-autocompletetype="given-name" required>
    			</div>
    		</div>
    		...
    		<div class="form-group">
    			...
    			<div class="col-sm-10">
    				<input class="form-control" id="inputNachname" name="nachname" type="text" value="<?php echo htmlentities($user['nachname']); ?>" x-autocompletetype="family-name" required>
    			</div>
    		</div>
        <div class="form-group">
    			<div class="col-sm-10">
            <label for="inputFestnetznummer">Festnetznummer (optional):</label>
            <input type="text" id="inputFestnetznummer" name="festnetz" class="form-control" value="<?php echo htmlentities($user['festnetz']); ?>" x-autocompletetype="tel">
          </div>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" id="inputMobilnummer" name="mobil" class="form-control" value="<?php echo htmlentities($user['mobil']); ?>" x-autocompletetype="tel" required>
    			</div>
        </div>
    ...
    <div role="tabpanel" class="tab-pane" id="passwort">
    	...
    	<form action="?save=passwort" method="post" class="form-horizontal" accept-charset="utf-8">
    		<div class="form-group">
    			...
    			<div class="col-sm-10">
    				<input class="form-control" id="inputPasswort" name="passwortAlt" type="password" x-autocompletetype="current-password" required>
    			</div>
    		</div>
    		...
    		<div class="form-group">
    			...
    			<div class="col-sm-10">
    				<input class="form-control" id="inputPasswortNeu" name="passwortNeu" type="password" x-autocompletetype="new-password" required>
    			</div>
    		</div>
    		...
    		<div class="form-group">
    			...
    			<div class="col-sm-10">
    				<input class="form-control" id="inputPasswortNeu2" name="passwortNeu2" type="password" x-autocompletetype="new-password" required>
    			</div>
    		</div>
